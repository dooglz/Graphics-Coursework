...
glm::vec3 GetForwardVector(glm::quat &q) {
  return glm::vec3(2.0f * (q.x * q.z + q.w * q.y), 2.0f * (q.y * q.z - q.w * q.x),
                   1.0f - 2.0f * (q.x * q.x + q.y * q.y));
}

glm::vec3 GetUpVector(glm::quat &q) {
  return glm::vec3(2 * (q.x * q.y - q.w * q.z), 1.0f - 2.0f * (q.x * q.x + q.z * q.z),
                   2.0f * (q.y * q.z + q.w * q.x));
}

glm::vec3 GetRightVector(glm::quat &q) {
  return glm::vec3(1.0f - 2.0f * (q.y * q.y + q.z * q.z), 2.0f * (q.x * q.y + q.w * q.z),
...
glm::vec4 CameraSpacePlane(const glm::mat4 &camproj, const glm::vec3 &pos, const glm::vec3 &normal,
  ...
  glm::vec3 cnormal = glm::normalize(glm::vec3(camproj * glm::vec4(normal, 1.0f))) * sideSign;
...
void CalculateObliqueMatrix(glm::mat4 &projection, const glm::vec4 &clipPlane) {
  glm::vec4 q =
      glm::inverse(projection) * glm::vec4(sgn(clipPlane.x), sgn(clipPlane.y), 1.0f, 1.0f);
