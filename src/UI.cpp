...
static void Menu() {
  ...
  if (ImGui::CollapsingHeader("Enviroment")) {
    ...
    ImGui::Checkbox("Enable Time", &Enviroment::enabled);
    ImGui::SliderFloat("Day/Night Speed", &Enviroment::speed, 0, 1.0f, "%.4f", 0.8f);
    ImGui::SameLine();
    if (ImGui::Button("Reset")){ Enviroment::speed = 0 ;}
...
void UpdateUI() {
  ...
  if (newkeystate && newkeystate != keystate) {
    showMenu = !showMenu;
    if (showMenu) {
      glfwSetInputMode(graphics_framework::renderer::get_window(), GLFW_CURSOR,
