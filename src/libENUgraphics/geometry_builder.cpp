...
void generate_tb(geometry &geom, const std::vector<glm::vec3> &normals) {
  ...
  for (unsigned int i = 0; i < geom.get_vertex_count(); ++i) {
    ...
    binormal_data.push_back(
        glm::normalize(glm::cross(normals[i], tangent_data[i])));
  }
...
geometry geometry_builder::create_disk(const unsigned int slices,
                                       const glm::vec2 &dims) {
  // Type of geometry generated will be triangles
  geometry geom;
  ...
  tex_coords.push_back(
      glm::vec2(tex_coords[0].x + pos.x, tex_coords[0].y - pos.z));
  colours.push_back(glm::vec4(0.7f, 0.7f, 0.7f, 1.0f));
  // Angle per slice
  ...
  for (unsigned int i = 1; i < slices + 1; ++i) {
    ...
    pos = glm::vec3(cos(i * delta_angle) * dims.x / 2.0f, 0.0f,
                    -sin(i * delta_angle) * dims.y / 2.0f);
    // Recalculate minimal and maximal
    minimal = glm::min(minimal, pos);
    maximal = glm::max(maximal, pos);
...
geometry geometry_builder::create_cylinder(const unsigned int stacks,
  ...
  for (unsigned int i = 1; i <= slices; ++i) {
    ...
    tex_coords.push_back(
        glm::vec2(tex_coord.x + prev_vert.x, tex_coord.y - prev_vert.z));
    tex_coords.push_back(
        glm::vec2(tex_coord.x + curr_vert.x, tex_coord.y - curr_vert.z));
  ...
  auto circ = glm::pi<float>() *
              ((3.0f * (dims.x + dims.z)) -
               (sqrtf((3.0f * dims.x + dims.z) * (dims.x + 3.0f * dims.z))));
  ...
  for (unsigned int i = 0; i < stacks; ++i) {
    ...
    for (unsigned int j = 0; j < slices; ++j) {
      // Caculate vertices
      verts[0] = glm::vec3(cos(j * delta_angle), 1.0f - (delta_height * i),
                           sin(j * delta_angle));
      verts[1] =
          glm::vec3(cos((j + 1) * delta_angle), 1.0f - (delta_height * i),
                    sin((j + 1) * delta_angle));
      verts[2] =
          glm::vec3(cos(j * delta_angle), 1.0f - (delta_height * (i + 1)),
                    sin(j * delta_angle));
      verts[3] =
          glm::vec3(cos((j + 1) * delta_angle), 1.0f - (delta_height * (i + 1)),
                    sin((j + 1) * delta_angle));
      // Scale by 0.5 * dims
      for (auto &v : verts)
        v *= dims * 0.5f;
      ...
      coords[0] = glm::vec2((-delta_width * j) / glm::pi<float>(),
                            dims.y - ((delta_height * i * dims.y) / 2.0f));
      coords[1] = glm::vec2((-delta_width * (j + 1)) / glm::pi<float>(),
                            dims.y - ((delta_height * i * dims.y) / 2.0f));
      coords[2] =
          glm::vec2((-delta_width * j) / glm::pi<float>(),
                    dims.y - ((delta_height * (i + 1) * dims.y) / 2.0f));
      coords[3] =
          glm::vec2((-delta_width * (j + 1)) / glm::pi<float>(),
                    dims.y - ((delta_height * (i + 1) * dims.y) / 2.0f));

      // Triangle 1
      positions.push_back(verts[0]);
      normals.push_back(
          glm::normalize(glm::vec3(verts[0].x, 0.0f, verts[0].z)));
      tex_coords.push_back(coords[0]);
      positions.push_back(verts[3]);
      normals.push_back(
          glm::normalize(glm::vec3(verts[3].x, 0.0f, verts[3].z)));
      tex_coords.push_back(coords[3]);
      positions.push_back(verts[2]);
      normals.push_back(
          glm::normalize(glm::vec3(verts[2].x, 0.0f, verts[2].z)));
      ...
      normals.push_back(
          glm::normalize(glm::vec3(verts[1].x, 0.0f, verts[1].z)));
...
geometry geometry_builder::create_sphere(const unsigned int stacks,
                                         const unsigned int slices,
                                         const glm::vec3 &dims) {
  // Type of geometry generated will be triangles
  geometry geom;
  ...
  for (unsigned int i = 0; i < stacks; ++i) {
    ...
    for (unsigned int j = 0; j < slices; ++j) {
      ...
      verts[0] = glm::vec3(dims.x * -sin(theta) * sin(rho),
                           dims.y * cos(theta) * sin(rho), dims.z * cos(rho));
...
geometry geometry_builder::create_torus(const unsigned int stacks,
                                        const unsigned int slices,
  ...
  for (unsigned int i = 0; i < stacks; ++i) {
    ...
    for (unsigned int j = 0; j <= slices; ++j) {
      ...
      // Vertex 0
      verts[0] = glm::vec3(sin(a0) * r, sin(j * delta_slice) * ring_radius,
                           cos(a0) * r);
      norms[0] = glm::normalize(glm::vec3(sin(a0) * c, cos(a0) * c, sin(b)));
      coords[0] =
          glm::vec2((static_cast<float>(i) / static_cast<float>(stacks)) *
                        outer_circ / glm::pi<float>(),
                    (static_cast<float>(j) / static_cast<float>(slices)) *
                        ring_circ / glm::pi<float>());
      // Vertex 1
      verts[1] = glm::vec3(sin(a1) * r, sin(j * delta_slice) * ring_radius,
                           cos(a1) * r);
      norms[1] = glm::normalize(glm::vec3(sin(a1) * c, cos(a1) * c, sin(b)));
      coords[1] =
          glm::vec2((static_cast<float>(i + 1) / static_cast<float>(stacks)) *
                        outer_circ / glm::pi<float>(),
                    (static_cast<float>(j) / static_cast<float>(slices)) *
                        ring_circ / glm::pi<float>());
      // Vertex 2
      ...
      r = c + outer_radius;
      verts[2] = glm::vec3(
          sin(a0) * r, sin((j + 1) * delta_slice) * ring_radius, cos(a0) * r);
      ...
      // Vertex 3
      verts[3] = glm::vec3(
          sin(a1) * r, sin((j + 1) * delta_slice) * ring_radius, cos(a1) * r);
...
void geometry_builder::create_plane_r(const unsigned int width,
  ...
  for (unsigned int x = 0; x < width; ++x) {
    for (unsigned int z = 0; z < depth; ++z) {
      ...
      verts[0] = glm::vec3(-static_cast<float>(width) / 2.0f + x, 0.0f,
                           static_cast<float>(depth) / 2.0f - z);
      verts[1] = glm::vec3(-static_cast<float>(width) / 2.0f + (x + 1), 0.0f,
                           static_cast<float>(depth) / 2.0f - z);
      verts[2] = glm::vec3(-static_cast<float>(width) / 2.0f + x, 0.0f,
                           static_cast<float>(depth) / 2.0f - (z + 1));
      verts[3] = glm::vec3(-static_cast<float>(width) / 2.0f + (x + 1), 0.0f,
                           static_cast<float>(depth) / 2.0f - (z + 1));
...
geometry geometry_builder::create_plane(const unsigned int width,
                                        const unsigned int depth,
                                        const bool uvscale) {
  // Type of geometry generated will be triangles
  geometry geom;
  ...
  for (unsigned int x = 0; x < width; ++x) {
    for (unsigned int z = 0; z < depth; ++z) {
      ...
      float df = static_cast<float>(depth);
      // Triangle 1
      positions.push_back(verts[0]);
      if (uvscale) {
        tex_coords.push_back(glm::vec2(x / wf, z / df));
      } else {
        tex_coords.push_back(glm::vec2(x, z) / 10.0f);
      }
      positions.push_back(verts[3]);
      if (uvscale) {
        tex_coords.push_back(glm::vec2((x + 1) / wf, (z + 1) / df));
      } else {
        tex_coords.push_back(glm::vec2(x + 1, z + 1) / 10.0f);
      }
      ...
      positions.push_back(verts[2]);
      if (uvscale) {
        tex_coords.push_back(glm::vec2(x / wf, (z + 1) / df));
      } else {
        tex_coords.push_back(glm::vec2(x, z + 1) / 10.0f);
      }
      ...
      positions.push_back(verts[1]);
      if (uvscale) {
        tex_coords.push_back(glm::vec2((x + 1) / wf, z / df));
      } else {
        tex_coords.push_back(glm::vec2(x + 1, z) / 10.0f);
      }
