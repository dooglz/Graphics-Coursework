...
public:
  ...
  */
  texture(const std::string &filename, bool mipmaps,
          bool anisotropic) throw(...);
  // Creates a texture from the colour data provided
  texture(const std::vector<glm::vec4> &data, GLuint width,
          GLuint height) throw(...);
  /*
  Creates a texture from the colour data provided and with user defined
  mipmaps and anisotropic filtering
  */
  texture(const std::vector<glm::vec4> &data, GLuint width, GLuint height,
          bool mipmaps, bool anisotropic) throw(...);
